...
const emptyForm = {
  customer_code: "", full_name: "", service_type: "Website Development", sub_service_type: "",
  passport_type: "New passport", appointment_type: "Regular",
  appointment_date: "", birth_date: "", birth_place: "",
  week: "Week-1", month: "ሀምሌ", year: "2013",
  phone_number: "", email: "", passport_status: "Not started",
  application_no: "", reference_number: "", receipt_passport_number: "", amount_paid: "", date_of_issue: "", gender: "",
  served_by: "", marital_status: "Single", immigration_place: "", work_type: "",
  coming_platform: "", referral_details: "", url: "",
  address: "", remarks: "", smart_services_needed: false,
  birth_certificate_url: "", kebele_id_url: "", national_id_url: "",
  receipt_passport_picture_url: "",
  appointment_paper_url: "", photo_url: "", passport_url: "",
  court_document_url: "", police_report_url: "", receipt_url: "",
};
...
function Dashboard() {
  ...
  const [filter, setFilter] = useState({ name: "", phone: "", dob: "", appt: "" });
    ...
    <div className="max-w-[1200px] mx-auto space-y-6">
      ...
      <form onSubmit={submit} className="bg-card rounded-2xl shadow-sm border p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div><label className={labelCls}>Passport Type</label>
            <select className={inputCls} value={form.passport_type} onChange={(e) => set("passport_type", e.target.value)}>
              {PASSPORT_TYPES.map((s) => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div><label className={labelCls}>Appointment Type</label>
            <select className={inputCls} value={form.appointment_type} onChange={(e) => set("appointment_type", e.target.value)}>
          ...
          <div className="grid grid-cols-3 gap-2 md:col-span-3">
            <div><label className={labelCls}>Week</label>
              <select className={inputCls} value={form.week} onChange={(e) => set("week", e.target.value)}>
          ...
          <div><label className={labelCls}>Order Status</label>
            <select className={inputCls} value={form.passport_status} onChange={(e) => set("passport_status", e.target.value)}>
          ...
          <div><label className={labelCls}>Contacted By</label><input className={inputCls} placeholder="Agent Name" value={form.served_by} onChange={(e) => set("served_by", e.target.value)} /></div>
      ...
      <div className="bg-card rounded-2xl border overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="text-center py-10 text-muted-foreground">No customers yet. Register your first above.</td></tr>
            )}
            {filtered.map((c) => (
